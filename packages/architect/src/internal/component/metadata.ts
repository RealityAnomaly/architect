...
export class ComponentMetadata<TModel extends ComponentModel = ComponentModel> {
  ...
  >(clazz: ComponentClass<T>): ComponentMetadata<TModel> {
    return new ComponentMetadata<TModel>(
      Reflect.hasMetadata(Constants.MODEL_META_KEY, clazz)
        ? Reflect.getMetadata(Constants.MODEL_META_KEY, clazz)
        : undefined,
      Reflect.hasMetadata(Constants.TARGET_TYPE_META_KEY, clazz)
        ? Reflect.getMetadata(Constants.TARGET_TYPE_META_KEY, clazz)
        : undefined,
    );
  }
