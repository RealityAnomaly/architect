import type { Schema, SchemaTransformer } from "./types.ts";
import { escapeRegExp, omit, omitBy, uniq } from "@es-toolkit/es-toolkit";
import { Ajv, _ } from "ajv";
import standaloneCodeMod from "ajv/dist/standalone/index.js";
import assert from "node:assert";
import { formats } from "../../validate/index.ts";
...
const FORMAT_IMPORT = `import { formats } from "@glassway/architect/kubernetes/validate";\n`;
...
function omitKubernetesFields(schema: Schema): Schema {
  return omitBy(schema, (_v, k) => (k as string).startsWith("x-kubernetes-"));
}
...
function rewriteRuntimeImports(code: string): string {
  return code.replace(
    ...
    (_, name: string, runtimePath: string, property: string) => {
      const importPath = "@glassway/architect/kubernetes/validate/runtime/" + runtimePath;
