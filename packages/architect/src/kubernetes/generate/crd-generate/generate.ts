...
export async function generate(options: GenerateOptions): Promise<void> {
  // TODO: YAML version required?
  // @ts-ignore: override
  const data: CustomResourceDefinition[] = options.input
  ...
  const files = await generator(dedupeDefinitions(definitions), options.outputPath);
