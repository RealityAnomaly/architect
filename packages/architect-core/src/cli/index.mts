...
export class App {
  ...
  private async preSubcommand(thisCommand: commander.Command, actionCommand: commander.Command): Promise<void> {
    ...
    this.parent = await Architect.create(options.workspace, logLevel);
  ...
  private async compile(options: AppCommandCompileOptions) {
    ...
    if (options.target) {
      const target = await this.parent!.project!.getTarget(this.parent!, options.target);
      if (!target) {
      ...
      const targets = await this.parent!.project!.getTargets(this.parent!);
