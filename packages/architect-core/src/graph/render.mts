...
import * as graphviz from 'npm:ts-graphviz';
...
export class DependencyGraphRenderer {
  public static async render(input: DependencyGraph, options: DependencyGraphRenderOptions) {
    const graph = graphviz.digraph('cluster', {
      ...
      for (const [k, v] of Object.entries(input.components)) {
        ...
        if (v.component.parent) {
          ...
          parentGraph = parentGraph.subgraph('cluster_' + parent.rid, { label: parent.name });
        } else if (v.component.children.filter(c => c.independent).length > 0) {
          parentGraph = parentGraph.subgraph('cluster_' + v.component.rid, { label: v.component.name });
        };
    ...
    try {
      ...
      input.target.parent.logger.warn(`rendering dependency graph: failed to run unflatten: ${exception}`)
      input.target.parent.logger.warn('graph will not be flattened, and will look uglier')
    } finally {
      await fs.rm(dir, {
        force: true,
        recursive: true
    ...
    try {
      ...
      input.target.parent.logger.error(`rendering dependency graph: failed to run dot: ${exception}`);
    };
  };
};
