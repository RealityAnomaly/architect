...
export default function ({
  ...
  return async (definitions, root) => {
    return definitions.map((def) => {
      ...
      const schemaPath = getRelativePath(path, getSchemaPath(getDefinitionPath(def.schemaId)));
      let content = "";
      let comment = "";
      ...
      for (const ref of refs) {
        ...
        if (isAPIMachineryID(ref) && !isAPIMachineryID(def.schemaId)) {
          if (externalAPIMachinery) {
            imports.push({
              name,
              path: `@glassway/kubernetes-models/apimachinery/${trimPrefix(
                ref,
      ...
      if (def.schema.type === "object") {
        ...
        imports.push({
          name: "ModelData",
          path: "@glassway/kubernetes-types/model",
        });

        if (gvk) {
          imports.push({
            name: "TypeMeta",
            path: "@glassway/kubernetes-types/meta",
          });
...
constructor(data?: ModelData<${shortInterfaceName}>) {
  ...
  this.setDefinedProps(data);
        ...
        imports.push({ name: "Model", path: "@glassway/kubernetes-types/model" });
        imports.push({
        ...
        imports.push({
          name: "ValidateFunc",
          path: "@glassway/kubernetes-types/validate",
        });
