...
export class KubeCRDDependencyGraph {
  ...
  private constructor(
    result: Result,
    data: Record<string, KubeCRDRequirement>,
    options: KubeCRDDependencyOptions = {},
  ) {
    this.result = result;
    this.data = data;
  ...
  public validate() {
    ...
    Object.entries(this.data).forEach(([k, v]) => {
      ...
      Object.entries(this.data).forEach(([k2, v2]) => {
        if (k === k2) return;
        const both = v.exports.filter((r) =>
          v2.exports.findIndex((g) => r.compare(g)) > -1
        );
        ...
        this.result.graph.errors.push(
          new ValidationError(
            ...
            this.result.graph.target.toString(),
          ),
        );
      });
      ...
      if (missing.length > 0) {
        v.component.errors.push(
          new ValidationError(
            `attempted to use resources missing from cluster: ${
              missing.join(", ")
            }`,
            ValidationErrorLevel.ERROR,
            v.component.component.toString(),
          ),
        );
      }
    });
