...
export class HttpBuilder extends Builder {
  ...
  public async fetch(url: string, cache?: boolean, _options?: HttpFetchOptions): Promise<KubeResource[]> {
    if (cache) {
    ...
    if (response.status !== 200) {
      throw new Error(`HTTP fetch failed for ${url}: returned code ${response.status}`);
    };
