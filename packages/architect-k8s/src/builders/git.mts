...
import { KubeResource, walk } from 'jsr:@perdition/architect-core';
...
export class GitBuilder extends Builder {
  ...
  public async fetch(url: string, ref?: string, options?: GitFetchOptions): Promise<KubeResource[]> {
    ...
    try {
      ...
      const paths = options?.paths && options.paths.length > 0 ? options.paths.map(p => path.join(tempDir, p)) : [tempDir];
